...
这次主要不是加一个孤零零的新按钮，而是把一些“看得到却不好用”的地方接回真正的功能上。

工作台现在更像一张完整的 TAG 台面了：输出区可以直接编辑，TAG 会按类别上色，悬停能看说明，左键拖动换顺序，右键拖动调权重。完整 TAG 和无角色 TAG 还是分开的，不会丢。

OC 也靠近手边了。标题栏的 `+` 会在原地弹出角色选择，选完就出现在标题栏，不会把你丢到很远的资料库。点角色 chip 可以快速换服装、调 Order / Depth；右键可以从当前工作台移除，或者真的打开 OC 库去编辑。

历史记录放到了工作台下面，最近几次生成一眼能看到。想翻完整记录的时候再点“查看全部”，可以把输入、完整 TAG、无角色 TAG 都恢复回来。
