...
- `_start_tutorial()`：按需 11 步完整教程（tut_* 键），从快捷键面板「新手教程」打开，播放前确保工作台/提示词卡可见；各步挂 `OnboardingStep.on_enter` 回调跟手开合面板（第 2 步开设置/第 3 步收、第 7 步亮提示词卡、第 8 步开画师库/第 9 步收，`finished` 信号兜底收库）；首启 `_start_onboarding()` 不挂 on_enter，保持 8 步轻量不变
- `_register_hints()`：hint_send / hint_scrub 为里程碑触发（首次输入 / 首次生成完成），其余定时
